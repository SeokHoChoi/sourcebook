...
- 현재 브랜치 기준 최신 원격 `main` 반영 커밋: `8925c28 docs: 관측성과 데이터베이스 확장 학습 배치 추가 (#25)`
- 현재 적재 범위: 1장 p14~38
- 현재 읽기 배치 수: 13개
- 현재 누적 질문 이벤트 수: 29개
- 현재 트랙 phase: `1장 p14~38 적재 완료`

현재 적재된 1장 페이지는 아래와 같다.
...
현재 1장은 p38의 `데이터베이스의 규모 확장` 도입까지 들어갔다. p38 하단의 `수직적 확장` 문장은 다음 페이지로 이어지므로 임의 완성하지 않았다. 다음 캡처가 오면 `ch01-p38-database-scaling-intro`를 이어 확장하거나, p39 이후 분량이 충분하면 `데이터베이스의 규모 확장` 절의 새 페이지로 분리한다. 사용자가 중간 질문만 보낼 수도 있으므로, 새 배치가 아니더라도 `learner/events.ndjson`과 `study-guide.md`에 먼저 기록하고 나중에 원문 페이지와 연결할 수 있게 한다.
