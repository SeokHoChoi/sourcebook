...
- 현재 브랜치 기준 최신 원격 `main` 반영 커밋: `0c24958 docs: 무상태 웹 계층 학습 배치 추가 (#23)`
- 현재 적재 범위: 1장 p14~35
- 현재 읽기 배치 수: 10개
- 현재 누적 질문 이벤트 수: 24개
- 현재 트랙 phase: `1장 p14~35 적재 완료`
...
현재 1장은 p35까지 들어갔다. 다음 캡처는 목차상 `메시지 큐` p35 이후 내용일 가능성이 높다. p35의 메시지 큐 도입부는 `ch01-p35-message-queue-intro`에 별도 페이지로 들어가 있으므로, 다음 캡처가 이어지면 같은 절의 새 배치로 만들거나 기존 메시지 큐 설명과 자연스럽게 연결한다. 사용자가 중간 질문만 보낼 수도 있으므로, 새 배치가 아니더라도 `learner/events.ndjson`과 `study-guide.md`에 먼저 기록하고 나중에 원문 페이지와 연결할 수 있게 한다.
