...
현재 1장은 p29까지 들어갔다. 다음 캡처는 목차상 `무상태(stateless) 웹 계층` p30 이후일 가능성이 높다. 사용자가 중간 질문만 보낼 수도 있으므로, 새 배치가 아니더라도 `learner/events.ndjson`과 `study-guide.md`에 먼저 기록하고 나중에 원문 페이지와 연결할 수 있게 한다.
