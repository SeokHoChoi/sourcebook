...
- 현재 브랜치 기준 최신 원격 `main` 반영 커밋: `9d839af docs: CDN 학습 배치와 용어 풀이 추가 (#21)`
- 현재 적재 범위: 1장 p14~32
- 현재 읽기 배치 수: 8개
- 현재 누적 질문 이벤트 수: 17개
- 현재 트랙 phase: `1장 p14~32 적재 완료`
...
현재 1장은 p32까지 들어갔다. 다음 캡처는 목차상 `무상태(stateless) 웹 계층` 후반 또는 `데이터 센터` p33 이후일 가능성이 높다. 사용자가 중간 질문만 보낼 수도 있으므로, 새 배치가 아니더라도 `learner/events.ndjson`과 `study-guide.md`에 먼저 기록하고 나중에 원문 페이지와 연결할 수 있게 한다.
